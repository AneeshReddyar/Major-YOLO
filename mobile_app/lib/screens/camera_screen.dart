...
class _CameraScreenState extends State<CameraScreen>
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _statusMessage =
              isConnected ? "Ready" : "Backend server not connected";
  ...
  Future<void> _captureAndProcess() async {
    ...
    if (!_backendConnected) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              "Backend server not connected. Please reconnect and try again."),
          backgroundColor: AppTheme.errorColor,
        ),
      );
      return;
    ...
    try {
      ...
      final processResult =
          await _detectionService.processImage(File(imageFile.path));
  ...
  Future<void> _pickAndProcess() async {
    ...
    try {
      ...
      final processResult =
          await _detectionService.processImage(File(pickedFile.path));
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor:
                saved ? AppTheme.primaryColor : AppTheme.errorColor,
...
Expanded(
  child: _resultImageUrl != null
    ? Center(
        child: AspectRatio(
          aspectRatio: 0.75,  // Adjust this value based on your typical image aspect ratio
          child: Transform(
            alignment: Alignment.center,
            transform: Matrix4.rotationZ(pi/2), // 90 degrees clockwise
            child: Image.network(
